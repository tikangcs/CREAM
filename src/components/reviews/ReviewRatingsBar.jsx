...
class ReviewRatingsBar extends React.Component {
  ...
  clickLogger(input) {
    logInteraction(
      `review-filter-${this.props.value}-stars: ${input}`,
      "reviews"
    );
  }
  ...
  filterFunc(event) {
    ...
    if (this.props.width !== "0") {
      ...
      if (tempFilter.length > 0) {
        document.getElementById("reviews-reset-filters").style.display =
          "inline";
      } else {
  ...
  render() {
    return (
      <div className="review-bar">
        <div
          className={
            this.props.width !== "0" ? "bar-label-active" : "bar-label"
          }
          style={{
            color: Array.isArray(this.props.getReviews.filter)
              ? this.props.getReviews.filter.indexOf(this.props.value) >= 0
                ? "red"
                : "black"
              : "black",
          }}
        ...
        >
          <span>{this.props.value} Stars</span>
        </div>
        {
